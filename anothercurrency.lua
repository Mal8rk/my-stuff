...
local function checkMoneyInternal(counter, value)
    return counter._value == value
end

local function drawLevel(counter)
   if counter._id == 1 then
       if counter._value <= 9 then
	      Text.print(counter._value, 1, 552, 28)
	  else   
	      Text.print(counter._value, 1, 534, 28)
	  end   
   end
   if counter._id == 2 then
       if counter._value <= 9 then
	      Text.print(counter._value, 1, 318, 28)
	  else   
	      Text.print(counter._value, 1, 300, 28)
	  end   
   end
end


local function myLevelSelectDraw(counter, value)
    if counter._id == 1 then
	   Graphics.draw{
        type = RTYPE_TEXT,
        text = tostring(counter._value),
        priority = 8,
        x = 140,
        y = 78,
    }
...
function ac.registerCurrency(name, hijackDefaultCounter)
    ...
    local currency = {
        ...
        checkMoney = checkMoneyInternal,
		draw = drawLevel,
		drawLevelSelect = myLevelSelectDraw, -- Draws the coin counter. The function can be overridden and is not called internally. Default implementation is for debug purposes. myCurrency:draw()
